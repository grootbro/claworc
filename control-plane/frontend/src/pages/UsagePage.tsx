...
export default function UsagePage() {
  ...
  const comparisonSource = source === "agent" ? "gateway" : "agent";
  ...
  const { data: allTimeStats } = useUsageStats({
  ...
  const { data: comparisonStats } = useUsageStats({
    start_date: startDate,
    end_date: endDate,
  ...
  const byProvider = (stats?.by_provider ?? []).map((s) => ({
    ...
    _totalTokens: s.input_tokens + s.cached_input_tokens + s.output_tokens,
  }));
  const byModel = (stats?.by_model ?? []).slice(0, 10);
  const providerActivity = [...byProvider].sort((a, b) => b._totalTokens - a._totalTokens);
  ...
  const comparisonSourceLabel =
    comparisonStats?.meta?.source_label ??
    (comparisonSource === "gateway" ? "Gateway Logs" : "Agent Sessions");
  const notes = meta?.notes ?? [];
  const coverage = meta?.coverage;
  const skippedPreview = coverage?.skipped?.slice(0, 3) ?? [];
  const zeroCostProviders = byProvider.filter((provider) => provider._totalTokens > 0 && provider.cost_usd === 0);
  const zeroCostModels = byModel.filter((model) => (model.input_tokens + model.cached_input_tokens + model.output_tokens) > 0 && model.cost_usd === 0);
  const currentProviderKeys = new Set(byProvider.map((provider) => provider.provider_key));
  const comparisonProviders =
    comparisonStats?.by_provider
      ?.filter((provider) => provider.input_tokens + provider.cached_input_tokens + provider.output_tokens > 0 || provider.cost_usd > 0)
      .map((provider) => ({
        ...provider,
        _label: provider.provider_name || provider.provider_key,
      })) ?? [];
  const comparisonOnlyProviders = comparisonProviders.filter(
    (provider) => !currentProviderKeys.has(provider.provider_key),
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
        ...
        {comparisonStats && (comparisonStats.total.cost_usd > 0 || comparisonOnlyProviders.length > 0) ? (
          <div className="rounded-md bg-blue-50 border border-blue-200 px-3 py-2 text-sm text-blue-950">
            Also visible in {comparisonSourceLabel}: {formatCost(comparisonStats.total.cost_usd)} for this range
            {comparisonOnlyProviders.length > 0
              ? ` · providers not visible in this source: ${comparisonOnlyProviders
                  .slice(0, 4)
                  .map((provider) => provider._label)
                  .join(" · ")}`
              : ""}
            {" · Totals are kept separate here to avoid double-counting the same requests twice."}
          </div>
        ) : null}
        {zeroCostProviders.length > 0 ? (
          <div className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2 text-sm text-amber-900">
            Token activity without pricing in this source: {zeroCostProviders.map((provider) => provider._label).join(" · ")}
            {zeroCostModels.length > 0 ? ` · models: ${zeroCostModels.slice(0, 4).map((model) => model.model_id).join(" · ")}` : ""}
          </div>
        ) : null}
      </div>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-6 gap-4">
            {[
              ...
              { label: "All-time Cost", value: formatCost(allTimeStats?.total?.cost_usd ?? 0) },
            ].map(({ label, value }) => (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <h2 className="text-sm font-semibold text-gray-700 mb-3">Provider Activity (Tokens)</h2>
              {providerActivity.length === 0 ? (
                <div className="text-sm text-gray-400 py-8 text-center">No data</div>
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={providerActivity} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
