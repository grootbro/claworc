...
function normalizeRequestedNextStep(value) {
  ...
  if (normalized.includes("audit") || normalized.includes("аудит") || normalized.includes("review")) {
    return "подключить менеджера для аудита / architecture review";
  }
  if (normalized.includes("estimate") || normalized.includes("смет") || normalized.includes("оцен") || normalized.includes("price")) {
    return "подключить менеджера для rough estimate";
  }
  if (normalized.includes("scope") || normalized.includes("scoping") || normalized.includes("созвон") || normalized.includes("call") || normalized.includes("консультац")) {
    return "подключить менеджера для scoping call";
  }
  if (normalized.includes("appnative") || normalized.includes("seo") || normalized.includes("branding") || normalized.includes("marketing")) {
    return "подключить менеджера по AppNative / packaged execution";
  }
  if (normalized.includes("web3") || normalized.includes("blockchain") || normalized.includes("smart contract") || normalized.includes("nft") || normalized.includes("defi")) {
    return "подключить менеджера по Web3 / blockchain";
  }
  if (normalized.includes("ai") || normalized.includes("бот") || normalized.includes("automation") || normalized.includes("автомат")) {
    return "подключить менеджера по AI / automation";
  }
  if (normalized.includes("mvp") || normalized.includes("platform") || normalized.includes("mobile") || normalized.includes("app") || normalized.includes("site") || normalized.includes("website") || normalized.includes("build") || normalized.includes("разраб")) {
    return "подключить менеджера по custom build";
  }
...
function inferNextStep(input) {
  ...
  if (!text) return "";
  if (["audit", "аудит", "review"].some((token) => text.includes(token))) {
    return "подключить менеджера для аудита / architecture review";
  }
  if (["estimate", "смет", "оцен", "price"].some((token) => text.includes(token))) {
    return "подключить менеджера для rough estimate";
  }
  if (["scope", "scoping", "созвон", "call", "консультац"].some((token) => text.includes(token))) {
    return "подключить менеджера для scoping call";
  }
  if (["appnative", "seo", "branding", "marketing"].some((token) => text.includes(token))) {
    return "подключить менеджера по AppNative / packaged execution";
  }
  if (["web3", "blockchain", "smart contract", "nft", "defi"].some((token) => text.includes(token))) {
    return "подключить менеджера по Web3 / blockchain";
  }
  if (["бот", "automation", "автомат", "llm"].some((token) => text.includes(token)) || /\bai\b/.test(text)) {
    return "подключить менеджера по AI / automation";
  }
  if (["mvp", "platform", "mobile", "app", "site", "website", "build", "разраб", "прилож"].some((token) => text.includes(token))) {
    return "подключить менеджера по custom build";
  }
...
function buildSummary(record) {
  ...
  const nextStepLabel = nextStep
    ? nextStep.includes("брон")
      ...
      : nextStep.includes("audit")
        ? "нужен аудит / architecture review"
        : nextStep.includes("rough estimate")
          ? "нужен rough estimate"
          : nextStep.includes("scoping call")
            ? "нужен scoping call"
            : nextStep.includes("AppNative")
              ? "нужен AppNative / packaged route"
              : nextStep.includes("Web3 / blockchain")
                ? "нужен Web3 / blockchain разбор"
                : nextStep.includes("AI / automation")
                  ? "нужен AI / automation разбор"
                  : nextStep.includes("custom build")
                    ? "нужен custom build разбор"
  ...
  return [
    ...
    clean(record.units) ? `формат ${clean(record.units)}` : "",
    nextStepLabel,
  ]
    .filter(Boolean)
    .join(", ");
...
function inferPriority(record) {
  ...
  if (["менедж", "звон", "call", "audit", "аудит", "estimate", "смет", "оцен", "scope", "scoping", "web3", "blockchain", "automation", "автомат", "mvp", "build", "разраб"].some((token) => text.includes(token)) || /\bai\b/.test(text)) {
    return "высокий";
  }
  if (["appnative", "seo", "branding", "marketing", "консультац", "формат", "интеграц"].some((token) => text.includes(token))) {
    return "средний";
  }
  return "обычный";
}
...
function nextLeadId(records) {
  ...
  return `RF-${String(next).padStart(4, "0")}`;
}
...
function previewLeadForRouting(input) {
  ...
  const base = existing || {
    ...DEFAULTS,
    lead_id: requestedLeadId || "RF-preview",
    created_at: now(),
    updated_at: now(),
    status: "qualified",
    stage: "qualified",
...
function renderManagerCard(record) {
  ...
  const title = `Новый лид RaveFox IT Lab · ${record.lead_id} · ${titleName}`;
  const lines = [title];
  ...
  pushSection(lines, "Проект", [
    renderLine("Рынок / контекст", record.region),
    renderLine("Тип проекта", record.project_type),
    renderLine("Сценарий / use case", record.model_or_use_case),
    renderLine("Срок / горизонт", record.timeline),
    renderLine("Бюджет", record.budget),
    renderLine("Формат / объем", record.units),
  ]);
...
function customerConfirmation() {
  return "Готово. Я передал ваш запрос команде RaveFox IT Lab. Они свяжутся с вами здесь или в Telegram в ближайшее рабочее время.";
}
