...
function inferPriority(record) {
  ...
  if (["кастом", "custom", "quote", "смет", "call", "менедж", "звон"].some((token) => text.includes(token))) {
    return "высокий";
  }
  if (["подбор", "вариант", "сценар", "fit"].some((token) => text.includes(token))) {
    return "средний";
  }
  return "обычный";
}
...
function loadRegistry() {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      ...
      // Ignore malformed rows to keep the registry robust.
    }
  }
  return map;
}
...
function nextLeadId(records) {
  ...
  return `ND-${String(next).padStart(4, "0")}`;
}
