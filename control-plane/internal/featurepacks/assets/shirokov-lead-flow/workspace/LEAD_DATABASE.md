...
This workspace stores Shirokov Capital leads inside `workspace/leads`.

## Rules

- Lead ids use the format `SC-0001`.
- Do not renumber historical leads.
- Prefer updating the same active lead when the same person continues the same deal or selection.
- If the same contact explicitly opens a separate deal, create a new lead.

## Files

- `leads/SEQUENCE.txt` stores the next sequence seed
- `leads/registry.jsonl` stores compact structured records
- `leads/cards/SC-XXXX.md` stores readable lead cards
