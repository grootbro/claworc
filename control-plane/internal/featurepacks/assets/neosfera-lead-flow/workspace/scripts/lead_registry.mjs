...
function normalizeRequestedNextStep(value) {
  ...
  if (normalized.includes("кабинет") || normalized.includes("партнер") || normalized.includes("запуск")) {
    return "подключить менеджера по запуску кабинета / партнерству";
  }
  if (normalized.includes("обуч") || normalized.includes("оператор")) {
    return "подключить менеджера по обучению";
  }
  if (normalized.includes("диагност") || normalized.includes("auraskan") || normalized.includes("matrix")) {
    return "подключить менеджера для консультации по диагностике";
  }
  if (normalized.includes("сеанс") || normalized.includes("сесси") || normalized.includes("запис")) {
    return "подключить менеджера для записи на сессию";
  }
...
function inferNextStep(input) {
  ...
  if (!text) return "";
  if (text.includes("кабинет") || text.includes("партнер") || text.includes("запуск")) {
    return "подключить менеджера по запуску кабинета / партнерству";
  }
  if (text.includes("обуч") || text.includes("оператор")) {
    return "подключить менеджера по обучению";
  }
  if (text.includes("диагност") || text.includes("auraskan") || text.includes("matrix")) {
    return "подключить менеджера для консультации по диагностике";
  }
  if (text.includes("сеанс") || text.includes("сесси") || text.includes("запис")) {
    return "подключить менеджера для записи на сессию";
  }
...
function buildSummary(record) {
  ...
  const nextStepLabel = nextStep
    ? nextStep.includes("брон")
      ...
      : nextStep.includes("запуску кабинета") || nextStep.includes("партнерству")
        ? "нужен менеджер по запуску / партнерству"
        : nextStep.includes("обучению")
          ? "нужен менеджер по обучению"
          : nextStep.includes("диагностике")
            ? "нужна консультация по диагностике"
            : nextStep.includes("записи на сессию")
              ? "нужна запись на сессию"
...
function inferPriority(record) {
  ...
  if (["менедж", "звон", "call", "запис", "сеанс", "обуч", "оператор", "кабинет", "партнер", "запуск"].some((token) => text.includes(token))) {
    return "высокий";
  }
  if (["диагност", "auraskan", "matrix", "консультац", "город", "формат"].some((token) => text.includes(token))) {
    return "средний";
  }
  return "обычный";
}
...
function nextLeadId(records) {
  ...
  return `NS-${String(next).padStart(4, "0")}`;
}
...
function previewLeadForRouting(input) {
  ...
  const base = existing || {
    ...DEFAULTS,
    lead_id: requestedLeadId || "NS-preview",
    created_at: now(),
    updated_at: now(),
    status: "qualified",
    stage: "qualified",
...
function renderManagerCard(record) {
  ...
  const title = `Новый лид NeoSfera · ${record.lead_id} · ${titleName}`;
  const lines = [title];
  ...
  pushSection(lines, "Запрос", [
    renderLine("Город / география", record.region),
    renderLine("Направление", record.project_type),
...
function customerConfirmation() {
  return "Готово. Я передал ваш запрос команде NeoSfera. Они свяжутся с вами здесь или в Telegram в ближайшее рабочее время.";
}
